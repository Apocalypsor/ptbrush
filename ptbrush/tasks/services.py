...
from db import BrushTorrent, QBStatus, Torrent as TorrentDB
...
class PtTorrentService:
    ...
    @staticmethod
    def _insert_or_update_torrent(torrent: Torrent):
...
class BrushService:
    ...
    def get_brush_torrent(self, count: int = 10) -> List[Torrent]:
        ...
        torrents_db = (
            TorrentDB.select()
            .where((TorrentDB.free_end_time > now) & (TorrentDB.brushed == False))
            .order_by(TorrentDB.score.desc())
            .limit(count)
        )
        result = []
